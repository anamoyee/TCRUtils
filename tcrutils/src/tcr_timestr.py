...
def split_string_on_change(s: str) -> tuple[tuple[str]]:
  ...
  for i in range(len(s)-1): # Not using enumerate because i also need the next letter
    if (s[i].isalpha()) and not (s[i+1].isalpha()): # If there happens to be a NON-numer on the left and a number on the right...
...
def _split_string_on_change2(s: str) -> tuple[str]:
  splits = []
  for i in range(len(s)-1): # Not using enumerate because i also need the next letter
    if not s[i].isalpha() and s[i+1].isalpha(): # If there happens to be a NON-numer on the left and a number on the right...
      splits.append(i+1) # ...place a split point there
...
def seconds_until_time(target_time) -> tuple[int, bool]:
  ...
  return (int(seconds_until_target)+1, rolled_over) # +1 as a correction because it for some reason returned 1 second too little
...
""".replace('\n', '')
  ...
  def to_int(self, s: str, *, units: Mapping[str, int] | None = None, segment_splitter='!') -> int:
    ...
    def fix_from_tuple(x: tuple[int, bool] | int):
      if not isinstance(x, tuple): return int(x)
  ...
  def to_str2(self, seconds: int) -> str:
    ...
    if days >= 1: base_time_str = f'{days}d + ' + base_time_str
